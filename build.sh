...
build_monado() {
  ...
  # Vive HMD = built-in vive driver (default ON).
  # 6DoF lighthouse = steamvr_lh wrapping SteamVR's driver (SteamVR stays dead).
  # Survive stays OFF — worse tracking on this box.
...
build_xrizer() {
  ...
  if [[ ! -e "${PREFIX}/lib/xrizer/bin/linux64/vrclient.so" ]] \
    && [[ -e "${PREFIX}/lib/xrizer/libxrizer.so" ]]; then
    mkdir -p "${PREFIX}/lib/xrizer/bin/linux64"
    ln -sfn ../../libxrizer.so "${PREFIX}/lib/xrizer/bin/linux64/vrclient.so"
  fi
}
...
main() {
  echo "=== standalone build: Monado + xrizer (no Envision) ==="
  build_monado
  build_xrizer
